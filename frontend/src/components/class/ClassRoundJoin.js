...
const ClassRoundJoin = () => {
    ...
    const handletotalTimeBlur = () => {
        const lessonRoundDataSetCopy = new Array(lessonTotalRound).fill(
            initialLessonRoundItem
        );
        ...
        console.log(
            lessonRoundDataSetCopy[0].lessonRoundEndDateTime,
            "끝난 시간"
        );
    ...
    const handleInsertLessonRoundTime = () => {
        ...
        lessonRoundDataSetCopy[0].lessonRunningTimeForEnd =
        ...
        for (let i = 1; i < lessonTotalRound; i++) {
            // 배열 절대 바꾸지 마라.

            if (i - 1 < weekNum) {
                standardDate[(i - 1) % weekNum].setDate(
                    standardDate[(i - 1) % weekNum].getDate()
                );
                lessonRoundDataSetCopy[i].lessonRoundStartDatetime = new Date(
                    standardDate[(i - 1) % weekNum]
                );
                ...
                standardDate[(i - 1) % weekNum].setDate(
                    standardDate[(i - 1) % weekNum].getDate() + 7
                );
                lessonRoundDataSetCopy[i].lessonRoundStartDatetime = new Date(
                    standardDate[(i - 1) % weekNum]
                );
    ...
    const getLessonData = (roundData, idx) => {
        ...
        const updatedItem = lessonRoundDataSetCopy.find(
            (item) => idx === item.lessonRoundNumber - 1
        );

        if (updatedItem) {
            updatedItem.lessonRoundTitle = lessonRoundTitle;
            updatedItem.lessonRoundFileOriginName = lessonRoundFileOriginName
                ? lessonRoundFileOriginName
                : null;
        }
        setLessonRoundDataSet(lessonRoundDataSetCopy);
