...
const StudentLessonRoomPage = ({ token }) => {
    ...
    console.log(sessionToToken, " token in lessonRoom");
    let remotes = [];
    ...
    function connect(token) {
        ...
        roomState.session
            .connect(token, { clientData: roomState.myUserName })
            .then(() => {
    ...
    function checkSomeoneShareScreen() {
        ...
        isScreenShared =
            roomState.subscribers.some((user) => user.isScreenShareActive()) ||
    ...
    function subscribeToStreamCreated() {
        roomState.session.on("streamCreated", (event) => {
            const subscriber = roomState.session.subscribe(
                event.stream,
                undefined
            );
